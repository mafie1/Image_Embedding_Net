...
class CustomDatasetMultiple(Dataset):
    def __init__(self, dir, transform, image_transform, mask_transform):
        ...
        for index, img in enumerate(self.images):
            ...
            boundary = np.array(skimage.segmentation.find_boundaries(mask, mode='thick'), dtype=np.int8)

            image = np.dstack((image, boundary))
            print(image.shape)
...
def test():
    ...
    plt.subplot(1, 2, 1)
    plt.title('Image')
    plt.imshow(np.array(img_example.permute(1, 2, 0)[:,:,0:3]))
